...
Nothing is read from the host either, so the next step asks you to name its
distribution. The details discovery would have collected stay empty until Praxis
...
not, you are asked to select a supported release for the host, and the setup
does not move on until you do. Praxis manages a host through its distribution:
packages, patches, rollback, mirrors and compliance are all expressed against
one, so a host with no supported mapping cannot be added. If none of the
supported releases applies, that host has to wait until its distribution is
supported.

If you skipped verification, there is nothing to map: choosing the distribution
yourself is the only way past this step, and Praxis records it as your
declaration rather than as something it read from the host.
