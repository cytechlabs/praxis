...
fleet control plane covering inventory, access, content, patching, and
compliance, and is meant to stay that way. A paid edition adds scale beyond the free host cap and
a set of governance controls.

The features that ship free are intended to remain free. The paid boundary is
fixed deliberately, so there is no bait-and-switch: nothing that is free today
moves behind the paywall later.
...
- The paid edition activates through a license applied to the **same
  deployment**: install the open-source build, run free up to the host cap, buy
  later, apply a license key, and the same install unlocks the paid host cap and
  governance controls.
...
- **Monitoring and reporting**: package reports, fleet operations, activity feed,
...
Licensing is **offline-first, with no call-home**. A license is a signed
...
# Paid-capable, bundling the private extension (internal build only). Drop the
...
with `--no-index`, so no package-index URL, token, or credential is ever passed as
