...
Praxis records a structured remediation workflow on top of compliance evidence: a failing evidence row can become a tracked remediation request, that request can be approved or rejected by a separate operator, an approved request can produce a structured plan preview, the plan can be explicitly acknowledged, and the read layer exposes a `ready_for_execution` gate for a future execution capability. The whole workflow is **non-executing today** — no host mutation, no command dispatch, no approval auto-execution. See [`remediation-workflow.md`](remediation-workflow.md) for the operator-facing walkthrough; see [`audit-schema.md`](audit-schema.md) for the wire format.
...
- The remediation workflow is **non-executing**: the audit trail proves intent and approval, not execution outcome. A future execution capability will add `compliance_remediation_execution.*` events that pair with each acknowledged-and-ready plan.
...
- `ready_for_execution=true` indicates a plan has cleared every metadata gate (current + acknowledged + not stale + executable plan kind + approved request) but does not by itself prove remediation occurred — only that the operator authorized it. Pair with the future execution events when they ship.
...
3. Demonstrate the filtering — e.g., all `command.exec` events with `outcome=denied` in the last 90 days for requirement 10.2.4.
...
- `status_label` / `verdict_reason_label` / `runner_label` — plain-language
