...
# Seed the synthetic demo fleet (idempotent — re-run any time).
...
were created. It survives `docker compose down` + a fresh `up` — just re-run it.
...
| Remediation | **Compliance → Remediation** (`/compliance/remediation`) | A **requested** remediation for the failing finding — the governed request → approve → plan flow. |

The rollback feasibility rows and the interactive rollback/remediation state
transitions are **produced by clicking through the UI** during the demo — the
...
[Support Matrix](support-matrix.md) · [EOL data](eol-data.md).
