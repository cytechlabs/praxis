    ...
    <div className="space-y-4">
                ...
                <tbody>
                  {sinks.map(s => (
                    <tr key={s.id} className="border-b border-border hover:bg-white/5">
                      <td className="py-2 pr-4 text-content">{s.name}</td>
                      <td className="py-2 pr-4"><Badge variant="info">{s.kind}</Badge></td>
                      <td className="py-2 pr-4 font-mono text-xs text-content-muted truncate max-w-xs">{s.target}</td>
                      <td className="py-2 pr-4 text-xs">
                        {s.hmac_secret_set ? <Badge variant="success">set</Badge> : <span className="text-content-subtle">-</span>}
                      </td>
                      ...
                      <td className="py-2 pr-4 text-right">
                        <div className="flex gap-1 justify-end">
                          <button
                            ...
                            className="p-1.5 rounded-md text-content-subtle hover:text-content hover:bg-white/5"
                          >
                            <Send size={14} />
                          </button>
                          <button
      ...
      <Modal open={showNew} onClose={() => setShowNew(false)} title="New audit sink" maxWidth="max-w-lg">
        <div className="space-y-3">
          <div>
            <label className="block text-xs uppercase tracking-wide text-content-subtle mb-1">Name</label>
            <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="prod-siem" />
          </div>
          <div>
            <label className="block text-xs uppercase tracking-wide text-content-subtle mb-1">Kind</label>
          ...
          <div>
            <label className="block text-xs uppercase tracking-wide text-content-subtle mb-1">Target</label>
          ...
          {form.kind === 'http' && (
            <div>
              <label className="block text-xs uppercase tracking-wide text-content-subtle mb-1">HMAC secret (optional)</label>
              <Input
                type="password"
      ...
      <Modal open={deliveriesSink !== null} onClose={() => { setDeliveriesSink(null); setDeliveries([]); }} title={`${deliveriesSink?.name} - recent deliveries`} maxWidth="max-w-3xl">
        {deliveries.length === 0 ? (
          <div className="text-content-subtle text-sm py-4 text-center">No deliveries yet.</div>
        ) : (
          <div className="overflow-x-auto max-h-96">
            <table className="w-full text-xs">
              <thead>
                <tr className="text-left uppercase tracking-wide text-content-subtle border-b border-border">
              ...
              <tbody>
                {deliveries.map(d => (
                  <tr key={d.id} className="border-b border-border">
                    <td className="py-2 pr-3 font-mono text-content-muted">#{d.event_id}</td>
                    <td className="py-2 pr-3">
                      <Badge variant={d.status === 'delivered' ? 'success' : d.status === 'dead_letter' ? 'danger' : d.status === 'pending' ? 'warning' : 'neutral'}>
                        {d.status}
                      </Badge>
                    </td>
                    <td className="py-2 pr-3 text-content-muted">{d.attempts}</td>
