  ...
  if (formats.length === 1) {
    return (
      <button
        ...
        disabled={exporting}
        className="bg-border hover:bg-border-strong text-content px-3 py-1.5 rounded flex items-center gap-1 text-sm disabled:opacity-50"
      >
        ...
        <div className="absolute right-0 mt-1 bg-surface-overlay border border-border/60 rounded shadow-lg z-10">
          {formats.map((f) => (
            <button
              key={f}
              onClick={() => doExport(f)}
              className="block w-full text-left px-4 py-2 text-sm text-content hover:bg-white/[0.03]"
            >
              {f.toUpperCase()}
            </button>
          ))}
