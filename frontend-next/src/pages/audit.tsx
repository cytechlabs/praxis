  ...
  return (
    <MainLayout>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            ...
            <code className="font-mono text-slate-300">docs/audit-schema.md</code> for the wire format.
            Configure external sinks (syslog / HTTP / file) under Settings → Audit Export.
          </p>
        </div>
        <div className="text-xs text-gray-500">{total.toLocaleString()} total events</div>
      </div>
      ...
      <Card className="mb-4">
        ...
        <CardBody>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div>
              <label className="block text-xs uppercase tracking-wide text-gray-500 mb-1">Action</label>
            ...
            <div>
              <label className="block text-xs uppercase tracking-wide text-gray-500 mb-1">Outcome</label>
            ...
            <div>
              <label className="block text-xs uppercase tracking-wide text-gray-500 mb-1">Actor User ID</label>
              <Input type="number" value={filters.actor_user_id} onChange={e => setFilters(f => ({ ...f, actor_user_id: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs uppercase tracking-wide text-gray-500 mb-1">System ID</label>
                ...
                <tbody>
                  {events.map(e => (
                    <tr
                      ...
                      className="border-b border-gray-900 hover:bg-white/5 cursor-pointer"
                    >
                      <td className="py-2 pr-4 text-xs text-gray-400">
                        {e.timestamp ? formatTimestamp(e.timestamp) : '-'}
                      </td>
                      <td className="py-2 pr-4 font-mono text-xs text-gray-300">{e.action}</td>
                      ...
                      <td className="py-2 pr-4 text-xs text-gray-400">
                        ...
                        {e.actor.ip && <span className="ml-2 text-gray-600">{e.actor.ip}</span>}
                      </td>
                      <td className="py-2 pr-4 text-xs text-gray-400">
                        {e.target.kind && <span className="text-gray-500">{e.target.kind}:</span>}{' '}
                        {e.target.id || '-'}
                      </td>
                    </tr>
                  ))}
      ...
      <Modal open={selected !== null} onClose={() => setSelected(null)} title="Event detail" maxWidth="max-w-2xl">
        <pre className="text-xs font-mono text-gray-200 whitespace-pre-wrap break-words max-h-96 overflow-auto">
