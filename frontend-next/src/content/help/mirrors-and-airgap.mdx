---
title: Mirrors & Airgap
description: Package mirrors, content channels and profiles, signing keys, and airgapped export/import.
section: Content & Airgap
order: 3.7
keywords: [mirror, channel, profile, content, airgap, offline, signing key, upstream trust, apply, source list]
---

# Mirrors & Airgap

Praxis can serve packages to your fleet from mirrors you control, and can move that content into fully disconnected (airgapped) environments. Everything lives under the **Content** nav group.

## Mirrors

`Content → Mirrors` lists the package repositories Praxis maintains. A mirror has a **package family** (`deb` or `rpm`) and a **source mode**:
...
- `Content → Channels` - a **channel** bundles one or more mirrors of the same package family, optionally pinned to a specific mirror run.
- `Content → Profiles` - a **profile** is the host-facing object. Hosts, groups, and smart groups **subscribe** to a profile. Each host resolves to a **single effective profile**; when subscriptions conflict, the host is flagged rather than guessing.
...
This is a procedure with real operational steps (key handling, transfer, verification); the in-app screens drive it, but the full runbook lives in `docs/airgap.md`.

## Airgap signing keys & trust pins

`Content → Airgap Keys` is where you manage the two sides of bundle trust - no database edits required.
...
- Airgap runbook: `docs/airgap.md`. Serviceable families: `docs/support-matrix.md`.
- Hardening guidance for offline/edge deploys: `docs/production-hardening.md`.
