---
title: Fleet & Hosts
description: Register, group, tag, and dynamically target systems.
section: Fleet
order: 2
keywords: [systems, hosts, groups, tags, smart groups, ca trust, fleet]
---
...
![All Systems](/help/all-systems.png)
...
- **Unreachable** - set by the fleet health checker after N consecutive SSH failures ([see Monitoring](/help/monitoring-and-alerts))

### Bulk actions

Select rows with the checkbox column for bulk operations: update status, change group, assign/remove tags, deploy CA trust, decommission. Every bulk action records a `FleetOperation` row visible in `Monitoring → Fleet Operations`.

## Static Groups

![System Groups](/help/system-groups.png)
...
![Smart Groups](/help/smart-groups.png)

A **Smart Group** is a saved rule (JSON AND/OR tree) that resolves to a list of systems, recomputed as the fleet changes. Pick `Fleet → Smart Groups → New Smart Group` and use the visual rule builder.
...
Tags are free-form labels. Unlike groups, a system can carry many tags. They're handy for transient classification (`needs-reboot`, `gpu`, `public-facing`). Manage them from `Fleet → All Systems → bulk tag` or per-system on the detail page. Smart group rules can match on tag membership (`tag in [gpu]`).
...
See [SSH & Security](/help/ssh-and-security) for CA rotation and revocation workflows.
