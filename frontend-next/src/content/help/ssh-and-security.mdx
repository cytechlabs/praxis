---
title: SSH & Security
description: SSH policies, host keys, CA-signed certs, command whitelisting, and approval workflows.
section: Security
order: 6
keywords: [ssh, security, ca, host keys, tofu, whitelist, approval, rotation, revoke, multi-level]
---
...
![SSH Security](/help/ssh-security.png)

`Security → SSH Security` defines per-system policy. Each policy bundles:
...
Review `Security → Host Key Changes` to approve or reject a changed key. Approving pins the new key; rejecting drops the connection pool entry for that host and blocks further connects until you investigate.
...
`Security → Command Execution` is the interactive shell for ad-hoc commands against one or more targets. Every command:
...
![Command Whitelist](/help/command-whitelist.png)

`Security → Command Whitelist` defines which commands are allowed and at what risk level. Each entry has:
...
![Approval Queue](/help/approval-queue.png)

When a command matches a whitelist entry with `requires_approval=true`, the execution is paused and a **CommandApproval** row is created. The requester gets an in-app notification; admins see the request in `Security → Approval Queue`.
...
`Security → Command History` is the audit trail of every executed command: who, what, where, when, exit code, duration. Filter by user, system, whitelist entry, status.

`Security → Command Metrics` rolls the same data up - top commands, top-risk commands, rejection rate, approval-wait distribution. Use it to tune the whitelist (frequently-rejected commands may need an explicit entry) and to catch drift in operator behaviour.
