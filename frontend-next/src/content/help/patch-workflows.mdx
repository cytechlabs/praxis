...
order: 3.5
keywords: [patch, policy, ring, wave, update plan, advisory, approval, reboot, rollback, dispatch, remediation]
---

# Patch Workflows

Praxis separates *inventory* (what's installed and updatable - see **Packages**) from *patching* (a governed, staged, approvable process). Everything below lives under the **Packages** nav group.

## Patch policies

`Packages → Patch Policies` defines **how** a set of hosts gets patched, not when a single job runs. A policy carries:
...
`Packages → Patch Advisories` holds native distribution advisories - Ubuntu USN, Debian security (DSA), and Red Hat updateinfo (RHSA). Praxis joins each advisory's fixed-package targets against host facts and installed packages to compute **per-host applicability** (`applicable` / `fixed` / `not_applicable` / `unknown`).
...
`Packages → Update Plans` is where a policy becomes a concrete, reviewable change. A plan is organized into **waves** that follow the policy's **ring** order: wave 1 targets the first ring, and later waves only proceed once earlier ones are healthy.
...
Approval queues for patch and command actions are reachable from **Commands → Approval Queue** and the plan/rollback surfaces themselves.
...
- Linux support matrix: `docs/support-matrix.md`. Production hardening: `docs/production-hardening.md`.
