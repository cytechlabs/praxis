...
description: Compliance policies, evidence, exports, and the remediation request/plan/dispatch lifecycle.
section: Compliance
order: 6.5
keywords: [compliance, cis, evidence, verdict, remediation, auditor, exports, policy, starter pack, applicability]
---

# Compliance Workflows

Compliance in Praxis is evidence-first: policies define checks, checks produce per-host evidence with a verdict, and failing evidence can be turned into an approved remediation. Everything lives under the **Compliance** nav group.

## Dashboard

`Compliance → Dashboard` rolls up the fleet's current verdicts from the latest evaluation of each enabled policy. Tiles link into the filtered evidence list. This is the "where do we stand right now" view for an operator or auditor.

## Policies and the starter pack

`Compliance → Policies` lists the compliance policies and their checks. Each check has a **kind** (shown as a product label, and an **Evaluator** family - *Package & fact evaluation* or *Host probe (SSH)*) that determines what evidence it needs:
...
> **A green verdict is not an attestation.** Praxis reports what its checks observed against host facts and probe evidence. It does not claim certifiable compliance. See the control mapping in `docs/compliance-map.md`.

## Evidence and exports

Evidence is viewable fleet-wide, per host (`Compliance → Systems`), and per policy (the policy's Evidence tab). Each row carries the check, the host, the **Status**, the plain-language **Reason**, the **Evaluator** family, and when it was evaluated.
...
`Compliance → Remediation` is the fleet rollup of remediation work. The lifecycle, driven from the request and execution detail pages:
...
- Remediation procedure detail: `docs/remediation-workflow.md`.
- Control mapping: `docs/compliance-map.md`. Audit event schema: `docs/audit-schema.md`.
