...
description: Manage SSH credentials with OpenBao (a Vault-compatible secrets service) as the secret store.
section: Security
order: 5
keywords: [credentials, vault, ssh, password, ssh_key, rotation, secrets]
---
...
`Security → All Credentials` is the master list. Each row is one credential with:
...
Click **Rotate** on a managed credential to generate a fresh password or key and push it to Vault atomically. The credential's `updated_at` advances and every System using that credential picks up the new secret on its next connection - no restart needed. Rotation records an audit entry visible in `Monitoring → Audit Logs`.

For ssh_key credentials, rotation generates a new keypair and writes it to Vault. Remember to deploy the new public key to the target systems before/after rotation; Praxis can do this via a job or as part of CA trust deployment (see [SSH & Security](/help/ssh-and-security)).

## Vault Management

![Vault Management](/help/vault-management.png)

`Security → Vault Management` surfaces secrets-service connection state and lets admins switch between the embedded service (bundled **OpenBao**, for simple deployments) and an external OpenBao/HashiCorp Vault cluster.
...
- Rotate credentials on a schedule; the M10 fleet-ops platform can auto-run rotation jobs against smart groups
- Audit access via `Monitoring → Audit Logs` filtered to `credential_change`
- The `credential_change` event type fires for every rotation / create / delete - subscribe a Slack alert config to it for security visibility ([Alerts & Webhooks](/help/monitoring-and-alerts#alerts-and-webhooks))
