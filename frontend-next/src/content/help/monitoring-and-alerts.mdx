...
description: Dashboard, activity, drift detection, and outbound webhooks.
section: Monitoring
order: 7
keywords: [monitoring, alerts, webhooks, slack, hmac, drift, baselines, activity, audit]
---
...
`Fleet → Dashboard` is the top-level operational view. Covered in [Getting Started](/help/getting-started); the short version is: StatCards for fleet posture at the top, HealthBanner for critical alerts, detail cards below for breakdowns.

The **Drifted Systems** StatCard (introduced in M10) counts systems that diverge from at least one enabled baseline. Click through to the drift matrix ([see below](#drift-detection)).

## Activity Feed

![Activity Feed](/help/activity-feed.png)

`Monitoring → Activity Feed` is a live event stream - system registrations, job starts/completes, credential changes, alert fires, approval decisions, anything interesting. It's the same data the `ActivitySidebar` polls every 15 s, just paginated and filterable. Use it for "what happened in the last hour?" incident triage.

## Audit Logs

![Audit Logs](/help/audit-logs.png)

`Monitoring → Audit Logs` is the **forensic** log - every mutating action, with the actor's user_id, old value, new value, and a SHA256 of the command where relevant. Unlike Activity Feed (which is about human-readable recent activity), Audit Logs is immutable and complete, intended for post-incident investigation and compliance.
...
`Monitoring → Analytics` provides fleet-wide time-series - patch compliance over time, failure rate, approval throughput. Useful for reporting to non-operators who want the big picture.
...
`Monitoring → Baselines → New Baseline`. The editor has three sections:
...
![Drift Matrix](/help/drift.png)

`Monitoring → Drift` shows the matrix - systems × baselines with a cell per pair showing the latest status icon. Green tick = compliant; red X = drifted; amber triangle = error; grey question mark = never evaluated. Click any drifted cell to open the drawer with the full list of failing rules.
...
`BaselineCheck` rows accumulate over time (100 systems × 10 baselines × daily = 1,000/day). Praxis keeps 90 days of history and a daily retention sweep at 03:00 UTC purges older rows. Latest-per-(baseline,system) is preserved regardless of retention.

### Pairing with alerts

Want Slack pings on drift? Create an AlertConfig subscribing to whichever events fire around check runs - today that's best done manually (drift-scoped event is on the roadmap). A lightweight pattern: schedule a daily `Report` job that reads `/drift/summary` via API and fires a job completion event with the count in the title.
